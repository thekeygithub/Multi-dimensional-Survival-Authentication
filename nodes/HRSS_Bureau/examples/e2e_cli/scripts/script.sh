...
installTestCC () {
         ...
         ORG=$2
         setGlobals $PEER $ORG
         peer chaincode install -n dtc1 -v 1.0 -p github.com/hyperledger/fabric/examples/chaincode/go/testCC/cmd >&log.txt
         res=$?
         cat log.txt
         verifyResult $res "Chaincode installation on peer peer${PEER}.org${ORG} has Failed"
         echo "===================== Chaincode is installed on peer${PEER}.org${ORG} ===================== "
...
instantiateTestCC () {
         ...
         # lets supply it directly as we know it using the "-o" option
         if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
                peer chaincode instantiate -o orderer0.guizhou.com:7050 -C $CHANNEL_NAME -n dtc1 -v 1.0 -c '{"Args":["Init","jack","0x123dgccbpomkwe"]}' -P "OR ('Org1MSP.peer','Org2MSP.peer')" >&log.txt
         else
                 peer chaincode instantiate -o orderer0.guizhou.com:7050 --tls --cafile $ORDERER_CA -C $CHANNEL_NAME -n dtc1 -v 1.0 -c '{"Args":["Init","jack","0x123dgccbpomkwe"]}' -P "OR ('Org1MSP.peer','Org2MSP.peer')" >&log.txt
 
         fi
         res=$?
         cat log.txt
         verifyResult $res "TraceCC instantiation on peer${PEER}.org${ORG} on channel '$CHANNEL_NAME' failed"
         echo "===================== RecordCC is instantiated on peer${PEER}.org${ORG} on channel '$CHANNEL_NAME' ===================== "
         echo
 }
installSuyuanCC () {
