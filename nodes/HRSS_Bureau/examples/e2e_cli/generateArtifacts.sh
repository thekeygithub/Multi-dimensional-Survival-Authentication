...
echo "crypto file exist"
else
mkdir crypto-config
fi

if [ -d "channel-artifacts" ];then
echo "channel file exist"
else
...
function replacePrivateKey () {
	...
	cp docker-compose-e2e-template.yaml docker-compose-e2e.yaml
        ...
        CURRENT_DIR=$PWD
        cd crypto-config/peerOrganizations/org1.guizhou.com/ca/
        PRIV_KEY=$(ls *_sk)
        cd $CURRENT_DIR
        sed $OPTS "s/CA1_PRIVATE_KEY/${PRIV_KEY}/g" docker-compose-e2e.yaml
        cd crypto-config/peerOrganizations/org2.guizhou.com/ca/
        PRIV_KEY=$(ls *_sk)
        cd $CURRENT_DIR
